<?php require 'header.php' ?>

  <body>

    <div class="background-colour background-colour-splash">

      <div class="nav caps">
        <a href="projects.php">projects</a>
        <a href="contact.php">contact</a>
      </div>


    <div class="main-centred">

      <div class="splash-content">

        <h1>izsi salmon</h1>

        <div class="splash-nav caps">
          <div class="splash-nav-flex">
            <a href="webdev.php?table=webProjects">Web Design</a>
            <i class="fas fa-circle"></i>
            <a href="graphic.php?table=graphicProjects">Graphic Design</a>
            <i class="fas fa-circle"></i>
            <a href="photog.php?table=photoProjects">Photography</a>
          </div>
        </div>

        <div class="splash-location text-secondary caps">Wellington, NZ</div>



        <div class="cta">
          <div><a href="projects.php" class="cta-text">explore</a></div>
        </div>

      </div>
    </div>
</div>

<img src="images/icon.png" alt="sting ray :)" class="ray">
<?php require 'footer.php'; ?>
